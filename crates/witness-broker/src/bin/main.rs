...
use witness_broker::{nostr_publisher, vantage_store, zangbeto_anchor};
use witness_types::{
...
async fn create_attestation(
    ...
    s.attestations.write().unwrap().insert(attest_id.clone(), attestation.clone());

    // Fail-open: open a Vantage witness round for peer-agent validation.
    {
        let a = attestation.clone();
        tokio::spawn(async move {
            if let Err(e) = vantage_store::open_round_for_attestation(&a, a.sim_receipt_id.as_deref()).await {
                tracing::debug!(attest_id = %a.attest_id, "open_round_for_attestation: {e}");
            }
        });
    }
    ...
    let (hash, attest_clone) = {
        ...
        match attestations.get_mut(&id) {
            Some(a) => {
                ...
                (a.canonical_hash(), a.clone())
            }
            None => return (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "not found"}))),
        }
    };

    // Publish to Nostr (kind 31020) and anchor to Zàngbétò — both fail-open.
    let nostr_event_id = nostr_publisher::publish_attestation(&attest_clone).await;
    let anchor_id = zangbeto_anchor::anchor_attestation(
        &id,
        &hash,
        nostr_event_id.as_deref(),
    ).await;

    // Persist the nostr_event_id and zangbeto_anchor back into the attestation.
    {
        let mut attestations = s.attestations.write().unwrap();
        if let Some(a) = attestations.get_mut(&id) {
            a.nostr_event_id  = nostr_event_id.clone();
            a.zangbeto_anchor = anchor_id.clone();
            a.status          = AttestationStatus::Anchored;
        }
    }
